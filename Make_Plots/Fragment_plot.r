...
bread_grob<-ggplotGrob(plot_bread_subgenome(tbl_bread))
...
pdf(file=paste0(plot_export_prefix, "Fragment_alignments.pdf"), height=9, width=(9))
	grid.arrange(Alabel, mis_grob, Blabel, length_grob, Clabel, bread_grob, Dlabel, emmer_grob, heights=c(1,0.1,1.2), widths=c(0.1,1.4,0.1,1), layout_matrix=hlay)
dev.off()
